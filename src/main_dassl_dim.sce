...
//                      //////////////////////////                             
    ...
    t0          = t_DCH(1);                 // Initial time for the discharge simulation
    t1          = getdate();                // Start execution time calculation
    // Problem solving by DASSL solver (discharge)
    W_DCH       = dassl([w0, wd0], t0, t_DCH, rtol, atol, res_dassl_dim);
...
//                        //////////////////////                               
    ...
    t0    = t_PAU(1);                       // Initial time for the pause simulation
    t1    = getdate();                      // Start execution time calculation
    // Problem solving by DASSL solver (pause)
    W_PAU = dassl([w0, wd0], t0, t_PAU, rtol, atol, res_dassl_dim);
...
//                   /////////////////////////////                            
    ...
    "color",cmap(ii,:));
    // Simulated data
    plot(-SIM(ii).Q(1:EOD(ii))/3600,SIM(ii).u_neg(1:EOD(ii)),"color",..
    cmap(ii,:),"thick",2);
end
