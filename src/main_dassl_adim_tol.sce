...
exec(path+'\res_dassl_adim_tol.sci');
...
v_rtol = [1e-05,1e-06,1e-07,1e-08,1e-09,1e-10,1e-11];    // Vector of relative tolerance
v_atol = [1e-06,1e-07,1e-08,1e-09,1e-10,1e-11,1e-12];    // Vector of absolute tolerance
EXP  = list();                                           // Experimental data
SIM_ = list();                                           // Simulated data
Call_DCH = zeros(size(rate,2),length(v_rtol));
Call_PAU = zeros(size(rate,2),length(v_rtol));
...
//                       // Experimental data //                               
//                       ///////////////////////                               
        if (rtol > 1e-10)||(rtol == 1e-10 && ii < 8)||(rtol == 1e-11 && ii < 4) then
...
//                   /////////////////////////////                            
            ...
            Call_DCH(ii,idx_tol) = %nan;
            Call_PAU(ii,idx_tol) = %nan;
        end
    end
end
